    ...
    for (;;)
    {
      ...
      while (xQueueReceive(gBusTxQueue, &tx, 0) == pdPASS)
      {
        ...
        if (len > 0)
        {
          ...
          // 1. DER TURNAROUND-DELAY: Gib dem ESP32 3 Millisekunden Zeit,
          // um sein eigenes Echo zu löschen und auf "Empfang" zu schalten!
          vTaskDelay(pdMS_TO_TICKS(3));

          // 2. DER HARDWARE-MUTE: Schalte das Ohr des STM32 auf Silizium-Ebene ab,
          // damit er keinen Overrun-Error durch sein eigenes Echo bekommt!
          USART1->CR1 &= ~USART_CR1_RE;

          // Jetzt feuern wir das Paket ab
          Serial1.write(outFrame, len);
          Serial1.flush();

          // 3. OHR WIEDER AN: Das Paket ist draußen, wir dürfen wieder zuhören.
          USART1->CR1 |= USART_CR1_RE;

          // 4. Mülleimer leeren (nur zur absoluten Sicherheit)
...
void setup()
{
  Serial.begin(115200);      // HMI / Debug
  Serial1.begin(9600);

  pinMode(REED_PIN, INPUT_PULLUP);
  for (uint8_t pin : HW390_PINS)
    pinMode(pin, INPUT_ANALOG);
  for (uint8_t pin : LDR_PINS)
    pinMode(pin, INPUT_ANALOG);
